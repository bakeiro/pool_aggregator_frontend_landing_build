...
async function main() {
  ...
  const results: Array<{ route: string; out: string; ok: boolean; error?: string }> = [];
  ...
  for await (const entry of walk(PAGES_DIR, {
    ...
    try {
      ...
      console.error(`  ❌  ${route.padEnd(30)} → ${message}`);
      results.push({ route, out: outPath, ok: false, error: message });
    }
  }
